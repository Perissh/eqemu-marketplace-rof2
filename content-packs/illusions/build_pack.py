#!/usr/bin/env python3
"""
Build illusions/pack.json from a reference EQEmu DB.

Player self-illusions. Filtered to:
  * targettype 6 (Self) -- so the clicky always illusions YOU, never a target/pet, and
  * race <= 732 -- post-RoF2 race models cast but render nothing on a RoF2 client.
Each clicky reuses the real inventory icon of a stock item that clicks the same
spell, where one exists (a configurable default covers the rest).
...
PACK = "illusions"
CATEGORY = "Illusions"
DEFAULT_BASE = 4002000
PRICE = 10
WHERE = "s.name LIKE 'Illusion:%' AND s.targettype=6 AND NOT (s.effectid1=58 AND s.effect_base_value1>732)"
