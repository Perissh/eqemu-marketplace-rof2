#!/usr/bin/env python3
"""
Build familiars/pack.json from a reference EQEmu DB.

Cosmetic companion pets that follow you. Filtered to familiars whose summoned pet
exists in the `pets` table -- MakePet does an exact type= match with no fallback,
so a familiar with no pet row summons nothing. Each clicky reuses the real icon of
a stock item that clicks the same spell where one exists.

Re-run against your own DB to re-validate. install.py consumes pack.json.
"""
...
PACK = "familiars"
CATEGORY = "Familiars"
DEFAULT_BASE = 4002300
PRICE = 10
WHERE = "s.name LIKE 'Familiar:%' AND s.teleport_zone IN (SELECT type FROM pets)"
